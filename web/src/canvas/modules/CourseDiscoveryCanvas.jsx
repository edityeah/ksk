  ...
  return (
    <div className="p-5 space-y-3">
      <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search courses, sectors, job roles…"
        className="w-full px-3 py-2 border border-bdr-light rounded-card outline-none focus:border-primary-500" />
      <div className="space-y-2">
        {filtered.map(c => (
          <div key={c.id} className="rounded-card border border-bdr-light bg-white p-3">
            ...
            <button className="mt-2 px-3 py-1.5 text-xs rounded-pill bg-primary-500 text-white">Apply</button>
          </div>
        ))}
