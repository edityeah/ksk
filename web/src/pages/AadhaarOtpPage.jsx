// KSK Aadhaar OTP — 6-digit OTP, split layout matching SwiftChat style.
...
export default function AadhaarOtpPage() {
  ...
  function handleChange(i, v) {
    ...
    const n = [...otp]; n[i] = d; setOtp(n); setStatus('idle')
    if (d && i < OTP_LEN - 1) refs.current[i + 1]?.focus()
  }
  function handleKey(i, e) { if (e.key === 'Backspace' && !otp[i] && i > 0) refs.current[i - 1]?.focus() }
  ...
  function boxStyle(d) {
    const base = { width: 44, height: 54, fontSize: 22, fontWeight: 700, borderRadius: 14, border: '2px solid', outline: 'none', textAlign: 'center' }
